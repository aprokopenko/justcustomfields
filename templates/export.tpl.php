						...
						<li class="jcf_export-content-type"><h3><span class="jcf_checkbox_block"><input type="checkbox" name="select_content_type" value="" class="jcf-select_content_type"  /></span><?php _e('Content type: ', JCF_TEXTDOMAIN); ?><?php echo $key; ?></h3>
							<input type="hidden" disabled="disabled" name="export_data[<?php echo $key; ?>]" value="<?php echo $key; ?>" />
								<?php foreach( $fieldsets[$key] as $fieldset_id =>$fieldset ) : ?>
									<input type="hidden" disabled='disabled' data-fieldset ="export_data_<?php echo $key; ?>_fieldsets_<?php echo $fieldset_id; ?>"  name="export_data[<?php echo $key; ?>][fieldsets][<?php echo $fieldset_id; ?>][title]" value="<?php echo $fieldset['title']; ?>" class="jcf_hidden_fieldset" />
									<div class="jcf_inner_box" id="jcf_fieldset_<?php echo $fieldset_id; ?>">
										<h3 class="header"><span class="jcf_checkbox_block"><input type="checkbox" name="choose_fieldset" value="export_data_<?php echo $key; ?>_fieldsets_<?php echo $fieldset_id; ?>" class="jcf-choose_fieldset" /></span><?php _e('Fieldset:', JCF_TEXTDOMAIN); ?> <span><?php echo $fieldset['title'];  ?></span></h3>
													...
													<?php if( !empty($fieldset['fields'])) : ?>
														<?php foreach($fieldset['fields'] as $field_id => $field) : ?>
															<tr id="field_row_<?php echo $field_id; ?>">
																<td class="check-column"><input type="checkbox" class="choose_field" name="choose_field[]" value="export_data_<?php echo $key; ?>_fieldsets_<?php echo $fieldset_id; ?>" id="export_data_<?php echo $key; ?>_fieldsets_<?php echo $fieldset_id; ?>_fields_<?php echo $field_id; ?>" /></td>
																<?php if( !empty($field_settings[$key][$field_id]) ):?>
																<?php foreach($field_settings[$key][$field_id] as $key_setting => $field_setting): ?>
																	<input type="hidden" disabled=disabled' data-fieldset ="export_data_<?php echo $key; ?>_fieldsets_<?php echo $fieldset_id; ?>" data-field = 'export_data_<?php echo $key; ?>_fieldsets_<?php echo $fieldset_id; ?>_fields_<?php echo $field_id; ?>'  name="export_data[<?php echo $key; ?>][fieldsets][<?php echo $fieldset_id; ?>][fields][<?php echo $field_id; ?>][<?php echo $key_setting; ?>]" value="<?php echo $field_setting; ?>" />
																<?php endforeach; ?>
																<?php endif; ?>
