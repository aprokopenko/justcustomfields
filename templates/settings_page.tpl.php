				...
				<div class="jcf_inner-tab-content" >
					...
					<div class="jcf_inner_content">
						<form action="<?php get_permalink(); ?>" id="jcform_multisite_settings" method="post" class="jcf_form_horiz">
							...
							<input type="radio" class="jcf_choose_settings" name="jcf_settings" value="db" id="jcf_read_db" <?php echo (empty($jcf_read_settings) || $jcf_read_settings == 'db' ? 'checked="checked"' : '');  ?>/><label for="jcf_read_db">Database. You can't edit or move settings without export/import features (default)</label><br />
							<input type="radio" rel="" class="jcf_choose_settings" name="jcf_settings" value="file" id="jcf_read_file" <?php echo (!empty($jcf_read_settings) && $jcf_read_settings == 'file' ? 'checked="checked"' : ''); ?>/><label for="jcf_read_file">Theme config file. Field configuration is saved to the current theme folder in json format and can be copied to another site easily.</label><br /><br />
							
							<input type="submit" class="button-primary" name="jcf_update_settings" value="<?php _e('Save', JCF_TEXTDOMAIN); ?>" />	
						</form>
					</div>
				</div>
			</div>
