<?php
	...
	function jcf_ajax_save_multisite_settings(){
		...
		$old_multisite_setting = get_site_option('jcf_multisite_setting');
		
		if( $old_multisite_setting ){
			update_site_option( 'jcf_multisite_setting', $multisite_setting );
		}else{
			add_site_option( 'jcf_multisite_setting', $multisite_setting );
		}
		$resp = array('status' => '1');
		jcf_ajax_reposnse($resp, 'json');
	}
