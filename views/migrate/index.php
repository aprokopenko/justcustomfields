...
<div class="wrap">
	...
	<p>We found out that you upgraded the plugin to the newer version. Your field settings needs to be upgraded to
		continue using the plugin.</p>
	<p>Please make sure <strong>you have a backup of your current settings</strong> (database dump if you store them in
		database or json config file).</p>
	...
	<?php if ( ! empty( $warnings ) ) : ?>
		<div class="jcf_well notice-warning">
			...
			<?php foreach ( $warnings as $ver => $warning ) : ?>
				<h4><strong>v<?php echo esc_html( $ver ); ?> upgrade</strong></h4>
				<?php echo esc_html( $warning ); ?>
			<?php endforeach; ?>

			<h4><u>If you're unable to update your theme templates - please downgrade to the previous version.</u></h4>
			<p>You can find previous versions on <a href="https://wordpress.org/plugins/just-custom-fields/developers/"
		...
		<div class="jcf_well <?php echo empty( $warnings ) ? 'notice-success' : 'notice-warning'; ?>">
			...
			<ul class="jcf_list">
				<?php foreach ( $migrations as $ver => $m ) : ?>
					<li>v<?php echo esc_html( $ver ); ?> upgrade</li>
				<?php endforeach; ?>
			</ul>
		</div>
	<?php endif; ?>
	...
	<form method="POST"
		  onsubmit="if(jcf_migrate_errors && !confirm('Are you sure you will be able to update your code?')) return false;">
