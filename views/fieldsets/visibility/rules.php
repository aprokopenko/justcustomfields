	...
	if ( ! empty( $visibility_rules ) && is_array( $visibility_rules ) ) :
		...
		<table class="wp-list-table widefat fixed fieldset-visibility-rules">
			...
			<?php foreach ( $visibility_rules as $rule_key => $rule ) :
				...
				?>
				<tr class="visibility_rule_<?php echo esc_attr( $rule['rule_id'] ); ?>">
					<td><?php echo esc_html( $row_index ++ ); ?></td>
					<td>
						<?php if ( $row_index > 2 ) : ?>
							<strong><?php echo esc_html( strtoupper( $rule['join_condition'] ) ); ?></strong><br/>
						<?php endif; ?>
						<?php echo $rule_text; ?>
					</td>
					<td>
						<a href="#" class="dashicons-before dashicons-edit edit-rule"
						   data-rule_id="<?php echo esc_attr( $rule['rule_id'] ); ?>"></a>
						<a href="#" class="dashicons-before dashicons-no remove-rule"
						   data-rule_id="<?php echo esc_attr( $rule['rule_id'] ); ?>"></a><?php ?>
					</td>
				</tr>
			<?php endforeach; ?>
			</tbody>
